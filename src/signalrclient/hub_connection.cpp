...
namespace signalr
{
    ...
    void hub_connection::invoke(const std::string& method_name, const signalr_value& arguments, std::function<void(const signalr_value&, std::exception_ptr)> callback) noexcept
    {
        if (!m_pImpl)
        {
            callback(signalr_value::array(), std::make_exception_ptr(signalr_exception("invoke() cannot be called on destructed hub_connection instance")));
            return;
        }
    ...
    void hub_connection::send(const std::string& method_name, const signalr_value& arguments, std::function<void(std::exception_ptr)> callback) noexcept
    {
        if (!m_pImpl)
        {
