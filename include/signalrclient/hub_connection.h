...
namespace signalr
{
    ...
    {
    public:
        typedef std::function<void __cdecl (const signalr_value&)> method_invoked_handler;
        ...
        SIGNALRCLIENT_API void invoke(const std::string& method_name, const signalr_value& arguments = signalr_value::array(), std::function<void(const signalr_value&, std::exception_ptr)> callback = [](const signalr_value&, std::exception_ptr) {}) noexcept;

        SIGNALRCLIENT_API void send(const std::string& method_name, const signalr_value& arguments = signalr_value::array(), std::function<void(std::exception_ptr)> callback = [](std::exception_ptr) {}) noexcept;
